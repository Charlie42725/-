    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-400">管理所有一番賞品牌</p>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-slate-800 rounded-lg p-6 border border-slate-700 mb-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-slate-300 mb-2">品牌名稱 *</label>
              <input
                type="text"
                required
                value={formData.name}
            ...
            <div>
              <label className="block text-slate-300 mb-2">品牌描述</label>
              <textarea
        ...
        <table className="w-full">
          <thead className="bg-slate-700">
            <tr>
              ...
              <th className="text-left px-6 py-4 text-slate-300 font-medium">名稱</th>
              <th className="text-left px-6 py-4 text-slate-300 font-medium">Slug</th>
              <th className="text-left px-6 py-4 text-slate-300 font-medium">描述</th>
          ...
          <tbody className="divide-y divide-slate-700">
            {brands.length === 0 ? (
              ...
              brands.map((brand) => (
                <tr key={brand.id} className="hover:bg-slate-700/50 transition-colors">
                  <td className="px-6 py-4 text-slate-300">{brand.id}</td>
                  <td className="px-6 py-4">
                    <div className="font-medium text-white">{brand.name}</div>
                  </td>
                  <td className="px-6 py-4">
                    <code className="text-sm text-orange-400 bg-slate-900 px-2 py-1 rounded">
                      {brand.slug}
                    </code>
                  </td>
